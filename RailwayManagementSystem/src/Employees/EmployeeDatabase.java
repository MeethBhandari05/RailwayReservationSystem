...
public class EmployeeDatabase {
	...
	{
		String insert="INSERT INTO `employees`(`ID`, `Name`, `Email`, `Tel`, "
				+ "`Salary`, `Position`) "
	...
	public static ArrayList<Employee> getAllEmployees(Database database) throws SQLException
	{
		...
		while(rs.next())
		{
			...
			e.setTel(rs.getString("Email"));
			e.setSalary(rs.getDouble("Tel"));
			e.setPosition(rs.getString("Salary"));
	...
	{
		String update="UPDATE `employees` SET `Name`='"+e.getName()+"',"
				+ "`Email`='"+e.getEmail()+"',`Tel`='"+e.getTel()+"',`Salary`='"+e.getSalary()+"',"
	...
	public static String[] getEmployeesName(Database database) throws SQLException
	{
		ArrayList<Employee> employees = getAllEmployees(database);
	...
	{
		Employee e= new Employee();
		String select="SELECT `ID`, `Name`, `Email`, `Tel`, `Salary`, "
				+ "`Position` FROM `employees` WHERE `Name` = '"+name+"';";
		ResultSet rs = database.getStatement().executeQuery(select);
		rs.next();
		e.setID(rs.getInt("ID"));
		e.setName(rs.getString("Name"));
